module.exports = function ({ api, models }) {
    ...
    const checkttDataPath = __dirname + '/../Sherni.X/commands/checktuongtac/';
    setInterval(async () => {
      ...
      try {
        if (day != day_now) {
            ...
            console.log('Sherni.X');
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
                    let storage = [], count = 1;
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || 'Sherni.X';
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ Sherni.X ]');
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ Sherni.X ]");
    ...
    //DEFINE DATLICH PATH
    const datlichPath = __dirname + "/../Sherni/commands/cache/datlich.json";

    //FUNCTION WORKS AS IT'S NAME, CRE: SHERNI.X
    const monthToMSObj = {
        1: 31 * 24 * 60 * 60 * 1000,
        2: 28 * 24 * 60 * 60 * 1000,
        3: 31 * 24 * 60 * 60 * 1000,
        ...
        for (el of temp) {
            ...
            if ("ATTACHMENT" in el) {
                ...
                for (a of el.ATTACHMENT) {
                    ...
                    out.attachment.push(fs.createReadStream(__dirname + `/../Sherni/commands/cache/${a.fileName}`));
                }
            }
            console.log(out);
